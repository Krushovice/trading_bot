...
def test_get_instruments_info_success(monkeypatch, bybit):
    ...
    instr = bybit.get_instruments_info("BTCUSDT")

    # 4) Проверяем, что получен кортеж из 4 элементов
    assert isinstance(instr, dict) and len(instr) == 4

    assert instr["min_qty"] == 0.1  # из "0.1"
    assert instr["tick_size"] == 0.5  # из "0.5"
    assert instr["qty_step"] == 0.01  # из "0.01"
    assert instr["price_decimals"] == 3  # из 3
